...
QtNodes::NodeDataType qtengine::While::dataType(QtNodes::PortType portType, QtNodes::PortIndex portIndex) const
{
	...
	case QtNodes::PortType::None:
		break;
	case QtNodes::PortType::In:
		ret = portIndex == 0 ? FlowController().type() : Type(types::ClassTypeManager::instance()->type(QMetaType::Bool)).type();
		break;
	case QtNodes::PortType::Out:
